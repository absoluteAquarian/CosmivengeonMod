...
namespace CosmivengeonMod.Players {
	public class DicePlayer : ModPlayer {
		...
		public int nebulaBuffsTimer;
		public int beetleBuffsTimer;
		...
		public override void SaveData(TagCompound tag) {
			tag["badShop"] = badShop;
			tag["goodShop"] = goodShop;
			tag["shopTimer"] = shopModifierTimer;
			tag["forceGravity"] = forceReversedGravity;
			tag["forceGravityTimer"] = forcedGravityTimer;
			tag["fish"] = fishDontWantMe;
			tag["fishTimer"] = fishTimer;
			tag["moreIF"] = moreIFrames;
			tag["moreIFTimer"] = moreIFrameTimer;
			tag["noDecay"] = noStaminaDecay;
			tag["noDecayTimer"] = nsdTimer;
			tag["lives"] = extraLives;
			tag["nebula"] = nebulaBuffsTimer;
			tag["beetle"] = beetleBuffsTimer;
			tag["godmode"] = godmodeTimer;
			tag["damage"] = buffDamageTimer;
			tag["ammo"] = endlessClipTimer;
			tag["mana"] = endlessManaTimer;
		}
		...
		public override void LoadData(TagCompound tag) {
			...
			nebulaBuffsTimer = tag.GetInt("nebula");
			beetleBuffsTimer = tag.GetInt("beetle");
		...
		public override void PostUpdateMiscEffects() {
			...
			if (nebulaBuffsTimer > 0) {
				Player.AddBuff(BuffID.NebulaUpDmg3, nebulaBuffsTimer, quiet: false);
				Player.AddBuff(BuffID.NebulaUpLife3, nebulaBuffsTimer, quiet: false);
				Player.AddBuff(BuffID.NebulaUpMana3, nebulaBuffsTimer, quiet: false);
			}
		}
