	...
	public static partial class DesolationModeBossAI{
		...
		private static void BoC_AttackCheck(NPC npc){
			...
			if(npc.Helper().Timer > timerMax){
				...
				int proj = MiscUtils.SpawnProjectileSynced(npc.Target().Center, Vector2.Zero,
			...
			if(npc.Helper().Timer2 == target - 60){
				var sound = CoreMod.Instance.PlayCustomSound(npc.Target().Center - new Vector2(0, 50 * 16), "Zap");
				sound.Volume *= 0.75f;
			}
			...
			if(npc.Helper().Timer2 > target){
				...
				for(int i = -8; i < 8; i++){
					...
					int proj = MiscUtils.SpawnProjectileSynced(position, Vector2.Zero,
