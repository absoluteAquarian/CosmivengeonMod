...
namespace CosmivengeonMod.API.Edits {
	internal class JewelDoubleJumpEdits : Edit {
		...
		private static void Player_GrappleMovement(ILContext il) {
			ILHelper.CommonPatchingWrapper(il, CoreMod.Instance, GrappleMovementPatch);
		}
		...
		private static void Player_PlayerFrame(ILContext il) {
			ILHelper.CommonPatchingWrapper(il, CoreMod.Instance, PlayerFramePatch);
		}

		private static bool GrappleMovementPatch(ILCursor c, ref string badReturnReason) {
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				badReturnReason = "Could not find jumpAgainUnicorn instruction sequence";
				return false;
			}
			...
			c.EmitIfBlock(out _, static () => editPlayer.GetModPlayer<AccessoriesPlayer>().oronitusJump.abilityActive,
				action: static cursor => {
					cursor.Emit(OpCodes.Ldarg_0);
					cursor.EmitDelegate<Action<Player>>(static player => player.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpAgain = true);
				},
				targetsToUpdate: afterBlock);
		...
		private static bool CarpetMovementPatch(ILCursor c, ref string badReturnReason) {
			...
			c.Emit(OpCodes.Ldarg_0);
			c.EmitDelegate<Func<Player, bool>>(static player => player.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpAgain);
		...
		private static bool JumpMovementPatch(ILCursor c, ref string badReturnReason) {
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				badReturnReason = "Could not find jumpAgainBlizzard instruction sequence";
				return false;
			}
			...
			c.EmitElseIfBlock(out _, branchAfterChecks, static () => editPlayer.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpAgain,
				action: static cursor => {
					cursor.Emit(OpCodes.Ldarg_0);
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				i => i.MatchLdcI4(1),
				i => i.MatchStfld(Player_jumpAgainUnicorn))) {
				badReturnReason = "Could not find jumpAgainUnicorn assignment instruction sequence";
				return false;
			}

			// Cache the related info for the branch targets
			TargetContext ctx = new(c);

			c.Emit(OpCodes.Ldarg_0);
			c.Emit(OpCodes.Stsfld, editPlayer_Field);

			c.EmitIfBlock(out _, static () => (editPlayer.velocity.Y == 0f || editPlayer.sliding || (editPlayer.autoJump && editPlayer.justJumped)) && editPlayer.GetModPlayer<AccessoriesPlayer>().oronitusJump.abilityActive,
				action: static cursor => {
					cursor.Emit(OpCodes.Ldarg_0);
					cursor.EmitDelegate<Action<Player>>(static player => player.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpAgain = true);
				});

			// Modify the branch targets
		...
		private static bool UpdatePatch(ILCursor c, ref string badReturnReason) {
			// This is where 'AccessoriesPlayer.oronitusJump.jumpAgain' is set or cleared
			FieldInfo Player_jumpAgainUnicorn = typeof(Player).GetField("jumpAgainUnicorn", BindingFlags.Public | BindingFlags.Instance);
			FieldInfo Player_doubleJumpUnicorn = typeof(Player).GetField("doubleJumpUnicorn", BindingFlags.Public | BindingFlags.Instance);
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				i => i.MatchStfld(Player_jumpAgainUnicorn),
				i => i.MatchBr(out _))) {
				badReturnReason = "Could not find jumpAgainUnicorn clearing instruction sequence";
				return false;
			}

			c.Index--;
			c.Emit(OpCodes.Ldarg_0);
			c.EmitDelegate<Action<Player>>(static player => player.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpAgain = false);
			 ...
			 *         <== here
			 */
			ILLabel notSlidingBranch = null;
			if (!c.TryGotoNext(MoveType.After, i => i.MatchBrfalse(out notSlidingBranch),
				i => i.MatchLdarg(0),
				i => i.MatchLdcI4(1),
				i => i.MatchStfld(Player_jumpAgainUnicorn))) {
				badReturnReason = "Could not find jumpAgainUnicorn assignment instruction sequence";
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				badReturnReason = "Could not find entity X position read instruction sequence";
				return false;
			}

			c.Index -= 2;
			c.Emit(OpCodes.Ldarg_0);
			c.EmitDelegate<Action<Player>>(static player => player.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpEffectActive = false);
			...
			c.EmitIfBlock(out _, static () => editPlayer.GetModPlayer<AccessoriesPlayer>().oronitusJump.jumpEffectActive && editPlayer.GetModPlayer<AccessoriesPlayer>().oronitusJump.abilityActive,
				action: static cursor => {
					cursor.Emit(OpCodes.Ldarg_0);
					cursor.EmitDelegate<Action<Player>>(static player => {
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				badReturnReason = "Could not find DashMovement call instruction sequence";
				return false;
			}
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				i => i.MatchStfld(Player_dJumpEffectUnicorn),
				i => i.MatchBr(out _))) {
				badReturnReason = "Could not find final dJumpEffectUnicorn assignment instruction sequence";
				return false;
			}
		...
		private static bool PlayerFramePatch(ILCursor c, ref string badReturnReason) {
			FieldInfo Player_dJumpEffectUnicorn = typeof(Player).GetField("dJumpEffectUnicorn", BindingFlags.Public | BindingFlags.Instance);
			...
			if (!c.TryGotoNext(MoveType.After, i => i.MatchLdarg(0),
				...
				i => i.MatchStfld(Player_dJumpEffectUnicorn),
				i => i.MatchLdloca(27))) {
				badReturnReason = "Could not find dJumpEffectUnicorn assignment instruction sequence";
				return false;
			}
