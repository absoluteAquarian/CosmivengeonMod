	...
	public static partial class DesolationModeBossAI{
		...
		public static void AI_EyeOfCthulhu(NPC npc){
			...
			if(Main.rand.NextBool(5)){
				int num9 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + npc.height * 0.25f), npc.width, npc.height / 2, 5, npc.velocity.X, 2f);
			...
			if(Main.dayTime || targetIsDead){
				...
				if(npc.ai[3] == -1f){
					...
					if(npc.ai[1] == 0f){
						...
						for(int i = 0; i < 20; i++){
							Dust dust = Dust.NewDustDirect(usePosition, width, height, 170, Scale: 1.5f);
							dust.noGravity = true;
						}
			...
			if(npc.Helper().Flag3 && !npc.Target().dead && DetourNPCHelper.EoC_FirstBloodWallNPC == npc.whoAmI){
				...
				for(corner.Y = origY; corner.Y <= otherCorner.Y; corner.Y++){
					...
					for(int i = 0; i < dustsPerTile; i++){
						Dust dust = Dust.NewDustDirect(spawn, 16, 16, 5, Scale: 2f);
						dust.noGravity = true;
					}
				}
		...
		private static void EyeOfCthulhu_SpawnSummon(NPC npc, Vector2 position, Vector2 velocity, bool belchSound = true){
			...
			for(int m = 0; m < 10; m++)
				Dust.NewDust(position, 20, 20, 5, velocity.X * 0.4f, velocity.Y * 0.4f);
		}
	}
}
