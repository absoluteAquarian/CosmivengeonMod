...
namespace CosmivengeonMod.NPCs.Draek{
	...
	public class DraekP2Head : Worm{
		...
		public static float GetHealthAugmentation() => CosmivengeonUtils.GetModeChoice(0, 0.6875f, 0.55f);
		...
		private void AI_MegaCharge(int delay, int flyTime, int rockCount, float targetTolerance){
			...
			if(attackProgress == 0){
				...
				speed = Main.player[npc.target].velocity.Length() + CosmivengeonUtils.GetModeChoice(6, 6, 9);
