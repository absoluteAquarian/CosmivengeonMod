	...
	public static class CheatSheet{
		...
		public static void Load(){
			if(ModReferences.CheatSheet.Active){ 
				...
				CosmivengeonMod.Instance.Logger.Error("Unable to patch CheatSheet since it doesn't exist.");
		}
		...
		private static void Patch_ctor(ILContext il){
			...
			c.EmitDelegate<Func<Texture2D>>(() => ModLoader.GetMod("CosmivengeonMod").GetTexture("UI/CheatSheet_NPCDamageResistance"));
...
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch CheatSheet.NPCBrowser..ctor()");
		}
		...
		private static void PatchDraw(ILContext il){
			...
			c.EmitDelegate<Func<NPC, string>>(npc => $"{npc.GetGlobalNPC<CosmivengeonGlobalNPC>().endurance :0.##}");
			c.Emit(OpCodes.Stelem_Ref);
			...
			//Using a label here because it's easier
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch CheatSheet.NPCBrowser.Draw()");
		}
