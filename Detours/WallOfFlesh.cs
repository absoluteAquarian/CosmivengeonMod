			...
			if(npc.Helper().Flag && Main.netMode != NetmodeID.MultiplayerClient){
				...
				if(npc.Helper().Timer2 <= WoF_Attack_EyeLasers)
					...
					for(int i = -3; i < 4; i++){
						...
						Projectile proj = Projectile.NewProjectileDirect(new Vector2(x, y), Vector2.UnitX * npc.direction * 1.85f, ProjectileID.DemonSickle, CosmivengeonUtils.TrueDamage(80), 0f, Main.myPlayer);
					...
					if(npc.Helper().Timer < 200){
						...
						for(float i = -3.5f; i <= 3.5f; i++){
							...
							Projectile proj = Projectile.NewProjectileDirect(pos, npc.Target().DirectionFrom(pos) * 9, ProjectileID.ImpFireball, CosmivengeonUtils.TrueDamage(66), 0f, Main.myPlayer);
							proj.friendly = false;
							proj.hostile = true;
							proj.tileCollide = false;
							proj.timeLeft = 6 * 60;
		...
		public static void AI_WallOfFleshEye(NPC npc){
			...
			if(npc.localAI[2] == 0f){
				...
				if(flag28){
					...
					Projectile proj = Projectile.NewProjectileDirect(vector35, new Vector2(num360, num361), num367, CosmivengeonUtils.TrueDamage(num366), 0f, Main.myPlayer);
					proj.tileCollide = false;
				}
			}
		}
	...
	public static partial class DesolationModeMonsterAI{
		public static void AI_TheHungry(NPC npc){
			...
			if(npc.ai[3] >= Main.rand.Next(350, 600)){
				...
				int dmg = CosmivengeonUtils.TrueDamage(50);
