...
namespace CosmivengeonMod.ModEdits{
	public static class Vanilla{
		public static void Load(){
			IL.Terraria.Player.JumpMovement += JumpMovementPatch;
			IL.Terraria.Player.Update += UpdatePatch;
			IL.Terraria.Player.PlayerFrame += PlayerFramePatch;
			IL.Terraria.Player.CarpetMovement += CarpetMovementPatch;
			IL.Terraria.Player.GrappleMovement += GrappleMovementPatch;
			//DoubleJumpVisuals is small enough that I can just detour it and add the custom jump's effects
			On.Terraria.Player.DoubleJumpVisuals += DoubleJumpVisualsDetour;
		}

		private static void GrappleMovementPatch(ILContext il){
			...
			c.EmitDelegate<Func<Player, bool>>(player => player.GetModPlayer<CosmivengeonPlayer>().doubleJump_JewelOfOronitus);
			...
			return;
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch Terraria.Player.GrappleMovement()");
		}

		private static void CarpetMovementPatch(ILContext il){
			FieldInfo Player_jumpAgainUnicorn = typeof(Player).GetField("jumpAgainUnicorn", BindingFlags.Public | BindingFlags.Instance);
			...
			return;
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch Terraria.Player.CarpetMovement()");
		}

		private static void DoubleJumpVisualsDetour(On.Terraria.Player.orig_DoubleJumpVisuals orig, Player self){
			orig(self);

			CosmivengeonPlayer mp = self.GetModPlayer<CosmivengeonPlayer>();
			if(mp.dJumpEffect_JewelOfOronitus && mp.doubleJump_JewelOfOronitus && !mp.jumpAgain_JewelOfOronitus && ((self.gravDir == 1f && self.velocity.Y < 0f) || (self.gravDir == -1f && self.velocity.Y > 0f))){
				if(self.height >= 32){
					for(int i = 0; i < 7; i++){
		...
		private static void JumpMovementPatch(ILContext il){
			...
			c.EmitDelegate<Func<Player, bool>>(player
				=> (player.velocity.Y == 0f || player.sliding || (player.autoJump && player.justJumped)) && player.GetModPlayer<CosmivengeonPlayer>().doubleJump_JewelOfOronitus);
			...
			return;
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch Terraria.Player.Update()");
		}
		...
		private static void UpdatePatch(ILContext il){
			...
			c.EmitDelegate<Action<Player>>(player => {
				...
				if(!mp.doubleJump_JewelOfOronitus)
					mp.jumpAgain_JewelOfOronitus = false;
				else if(player.velocity.Y == 0f || player.sliding)
					mp.jumpAgain_JewelOfOronitus = true;
			});
			...
			c.Index -= 2;
			c.Emit(OpCodes.Ldarg_0);
			c.EmitDelegate<Action<Player>>(player => player.GetModPlayer<CosmivengeonPlayer>().dJumpEffect_JewelOfOronitus = false);
			...
			c.Emit(OpCodes.Ldarg_0);
			c.EmitDelegate<Func<Player, bool>>(player => player.GetModPlayer<CosmivengeonPlayer>().dJumpEffect_JewelOfOronitus && player.GetModPlayer<CosmivengeonPlayer>().doubleJump_JewelOfOronitus);
...
bad_il:
		...
		private static void PlayerFramePatch(ILContext il){
			FieldInfo Player_dJumpEffectUnicorn = typeof(Player).GetField("dJumpEffectUnicorn", BindingFlags.Public | BindingFlags.Instance);
			...
			return;
bad_il:
			CosmivengeonMod.Instance.Logger.Error("Unable to fully patch Terraria.Player.PlayerFrame()");
		}
	}
}
